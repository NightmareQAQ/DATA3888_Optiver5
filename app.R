...
all_files_short <- dir_short[1:2] # Select only the first 20 stock files
...
ui <- navbarPage("Extended Shiny App", id = "tabs",
                ...
                .data-table-container {
                    background-color: #B0E0E6;  /* LightBlue */
                    padding: 10px;
                    border-radius: 10px;
                }
            "))
                          ...
                          fluidRow(
                            column(12, align = "center", 
                                   ...
                                   tags$h3("Optver"),
                                   tags$br(),
                                   awesomeCheckboxGroup("checkbox", label = NULL, choices = list("Stock" = "Stock"), selected = "Stock", inline = TRUE, status = "primary"),
                                   tags$br(),
                                   actionButton("goToTab1", "Monitor", class = "btn-primary", style = "width: 200px; height: 50px; font-size: 20px; margin: 10px;"),
                                   actionButton("goToTab2", "Model", class = "btn-primary", style = "width: 200px; height: 50px; font-size: 20px; margin: 10px;")
                            )
                            ,
                            #div(class = "data-table-container", dataTableOutput("homeTable"))
                 ...
                 tabPanel("Monitor",
                          fluidPage(
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                numericInput("stock_id", "Stock ID:", 8382),
                                numericInput("time_id_start", "Time ID Start:", 1),
                                numericInput("time_id_end", "Time ID End:", 10),
                          ...
                          sidebarLayout(
                            ...
                            mainPanel(
                              ...
                              div(
                                ...
                                plotOutput("ModelPlot", width = "50%"),
                                plotOutput("Model2Plot", width = "50%")
                              ),
                 ...
                 tabPanel("Model_Intro",
                          fluidRow(
                            column(4,
                                   tags$div(style = "border: 1px solid black; padding: 20px; margin: 10px;",
                                            h2("RE"),
                                            p("As the first approach in this project, the regression model was incorporated with the aim of analysing and modelling the volatility of stock prices for predictive analysis. Often noted as a statistical/numerical based method, it allowed us to model the relationship between volatility and external variables and represent how these variables impact volatility over time (Rishi, 2022). Through its implementation, it allowed our project to model and evince the relationship various factors have on volatility, ultimately allowing us to predict and forecast volatility beyond the past and present")
                                   )
                            ),
                            column(4,
                                   tags$div(style = "border: 1px solid black; padding: 20px; margin: 10px;",
                                            h2("HAV"),
                                            p("This HAV model was chosen to model and predict volatility as it directly works on the time series of realised volatility. As the second approach for this project, it takes into account the time scales present with volatility and incorporates this primarily with the assumption that specific time scales often result with different perceptions towards volatility and its factors. This often leads to what is known as an “additive volatility cascade” and via an autoregressive model, allows for predictions and forecasts of volatility across different aspects in time (Clements & Preve, 2019).
")
                                   )
                            ),
                            column(4,
                                   tags$div(style = "border: 1px solid black; padding: 20px; margin: 10px;",
                                            h2("ARMA-GARCH"),
                                            p("A combination of both ARMA and GARCH models were used as the third approach to the project for predicting volatility as it is specifically able to capture time-varying volatility. The model captures what is known as the ‘volatility clustering phenomenon’ where a period of high volatility is immediately followed by a period of low volatility and vice versa; a common theme in financial time series data (Cont, 2017). The ARMA aspect works to model and evaluate the autocorrelation and moving mean patterns in datasets, evaluating recurring patterns in volatility. The GARCH aspect captures the dynamics of volatility by implementing logged residuals as a predictor of current volatility, allowing for the modelling of conditional heteroskedasticity (non-constant volatility from past time periods) as volatility is time-dependent (Ghani & Rahim, 2023).
")
                                   )
                            )
                          )
                 ...
                 tabPanel("About",
                          fluidRow(
                            column(6, 
                                   textAreaInput("textBox", label = "Input text here", 
                                                 value = "Background", width = "100%")),
                            column(6,
                                   imageOutput("imageBox", width = "100%")
                            )
                          )
                 )
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_cluster, {
    ...
    mse_data_adj <- data.frame(
      Model = factor(rep(c("Linear Regression", "HAV"), )),
      MSE = c(MSE.lm, MSE.HAV),
      MAPE = c(MAPE.lm, MAPE.HAV)
    )
    # Reshape the data
    ...
    output$ModelPlot <- renderPlot({
      # Plot for MSE
      ggplot(data = mse_data_adj_long_mse, aes(x = Model, y = Value, fill = Metric)) + 
    ...
    output$Model2Plot <- renderPlot({
      # Plot for MAPE
      ggplot(data = mse_data_adj_long_mape, aes(x = Model, y = Value, fill = Metric)) + 
        geom_bar(stat = "identity", position = "dodge") +
        theme_minimal() +
        labs(x = "Model", y = "Value", fill = "Metric") +
  ...
  rolling <- function(train_volume, combined_data, threshold_per) {
    ...
    plot = ggplot(ploting_df, aes(x = 1:length(ploting_df$true_volatility))) +
      ...
      labs(title = "Comparison of Two Vectors", x = "Index", y = "Value", colour = "Vector") +
